...
export const panelsData = async (req,res)=>{
    ...
    if(walletSearchId === 'all'){
         ...
         wallet = await sequelize.query("SELECT (SELECT SUM(price) FROM app_invoice WHERE user_id= :userId AND pay = 'paid' AND type = 'income') as income,(select SUM(price) FROM app_invoice WHERE user_id= :userId AND pay = 'paid' AND type = 'expense') as expense from app_invoice WHERE user_id = :userId and pay = 'paid' group by income,expense",
        ...
        receivedMonth = await sequelize.query("SELECT  SUM(price) as incomeMonth FROM app_invoice WHERE user_id = :userId AND pay = 'paid' AND month(due_at) = :date AND type = 'income'",{
            replacements:{userId:userSession,date:(currentDate.getMonth()+1)},
            type:QueryTypes.SELECT
        })
        paidMonth = await sequelize.query("SELECT  SUM(price) as expenseMonth FROM app_invoice WHERE user_id = :userId AND pay = 'paid' AND month(due_at) = :date AND type = 'expense'",{
            replacements:{userId:userSession,date:(currentDate.getMonth()+1)},
            type:QueryTypes.SELECT
        })
    }else{
        wallet = await sequelize.query("SELECT (SELECT SUM(price) FROM app_invoice WHERE user_id= :userId AND pay = 'paid' AND type = 'income' AND wallet_id = :w) as income,(select SUM(price) FROM app_invoice WHERE user_id= :userId AND pay = 'paid' AND type = 'expense' AND wallet_id = :w) as expense from app_invoice WHERE user_id = :userId AND pay = 'paid' AND wallet_id = :w group by income,expense",
        ...
        receivedMonth = await sequelize.query("SELECT  SUM(price) as incomeMonth FROM app_invoice WHERE user_id = :userId AND pay = 'paid' AND month(due_at) = :date AND type = 'income' AND wallet_id = :w",{
            replacements:{userId:userSession,date:(currentDate.getMonth()+1),w:walletSearchId},
            type:QueryTypes.SELECT
        })
        paidMonth = await sequelize.query("SELECT  SUM(price) as expenseMonth FROM app_invoice WHERE user_id = :userId AND pay = 'paid' AND month(due_at) = :date AND type = 'expense' AND wallet_id = :w",{
            replacements:{userId:userSession,date:(currentDate.getMonth()+1),w:walletSearchId},
            type:QueryTypes.SELECT
        })
    }
